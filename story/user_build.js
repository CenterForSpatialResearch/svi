...
var countyCentroids = d3.json("../geoData/county_centroids.geojson")
var counties = d3.json("../geoData/counties_neighbors.geojson")
var stateCentroids = d3.json("../geoData/us-state-centroids.json")
var usOutline = d3.json("../geoData/simple_contiguous.geojson")
...
var states = d3.json("../geoData/simplestates.geojson")
...
function ready(counties,outline,centroids,sviData,states,sCentroids){ 
   ...
   // console.log(pub.combined)
    ...
    var stateGeos = stateGeoDictionary(states)
    pub.coordByState = stateGeos
    
    drawMap(pub.combined)
       
}
function stateGeoDictionary(data){
    var stateDict = {}
    for(var i in data.features){
        var gid = data.features[i].properties["STATEFP"]
        stateDict[gid]=data.features[i].geometry.coordinates
    }
    return stateDict
}
...
function drawMap(data){
  ...
  //   }
    ...
    //    console.log(map.getStyle().layers)
    map.on("mousemove","counties",function(c){
...
function getUserInfo(userCoordinates,userProperties,map){
    // d3.select("#user").style("display","inline")
//     d3.select("#neighbor").style("display","inline")
//     d3.select("#state").style("display","inline")
//     d3.select("#percentile").style("display","inline")
